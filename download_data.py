...
def convert_dataset(
    ...
    for split in ["train", "test"]:
        ...
        if max_episodes is not None:
            ...
            print(f"Limited to {max_episodes} episodes for {split}")
...
def main(
    ...
    fps: int = 20,
    max_episodes: int = None,  # 限制最大episode数量
    tfds_cache_dir: str = "./tfds_cache",  # TFDS 缓存目录
    cleanup_cache: bool = True,  # 下载完成后是否清理缓存
) -> None:
    # 设置 TFDS 缓存目录
    import os
    os.environ['TFDS_DATA_DIR'] = tfds_cache_dir
    print(f"TFDS cache directory set to: {tfds_cache_dir}")
    
    # 创建缓存目录
    Path(tfds_cache_dir).mkdir(parents=True, exist_ok=True)
    ...
    for dataset_name, dataset_config in selected_configs.items():
        convert_dataset(
            ...
            tfds_cache_dir=tfds_cache_dir,
        )
    
    # 清理缓存（可选）
    if cleanup_cache:
        print(f"Cleaning up TFDS cache directory: {tfds_cache_dir}")
        import shutil
        if Path(tfds_cache_dir).exists():
            shutil.rmtree(tfds_cache_dir)
            print("TFDS cache cleaned up successfully")
        else:
            print("No cache to clean up")
